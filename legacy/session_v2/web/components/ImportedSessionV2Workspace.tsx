import { ReplayValidationNotice } from './ReplayValidationNotice';
import { graphNodeLabels, MSGNET_GRAPH_NOTICE } from '@/data/graphSemantics';
...
export function ImportedSessionV2Workspace({ session }: { session: AuditSessionV2 }) {
  ...
  const variables = graphNodeLabels(session);
  ...
  return <div>
    <ReplayValidationNotice session={session}/>
    <div className="mx-auto grid max-w-[1400px] gap-6 px-5 py-12 lg:grid-cols-[270px_1fr_300px]">
      <aside className="card h-fit space-y-4 p-4"><div className="eyebrow">Imported Session v2 graph</div><Select value={sampleId} onChange={setSampleId} options={session.samples.map(item => ({ value: item.sample_index, label: `Test ${item.sample_index}` }))} ariaLabel="Imported v2 sample"/>{sample && <Select value={context.context_id} onChange={value => setContextId(value)} options={sample.contexts.map(item => ({ value: item.context_id, label: `${item.type} ${item.index}` }))} ariaLabel="Imported v2 context"/>}<p className="rounded-lg bg-[#f5f7fa] p-3 text-[10px] leading-relaxed text-ink-500">Blue marks the relation replayed offline.</p></aside>
      <main className="card min-h-[520px] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-[15px] font-semibold">Learned graph · {context?.type} {context?.index}</h3><p className="mt-1 text-[11px] text-[#176e69]">Stored graph with the audited relation fixed.</p></div><span className="font-mono text-[10px] text-ink-400">{context?.context_id}</span></div><div className="flex justify-center pt-8"><GraphNetwork variables={variables} edges={edges} layout="circular" showLabels threshold={0} target={selected?.target ?? 0} highlightTarget={selected !== null} selectedNode={null} selectedEdge={selected} size={460}/></div></main>
      <aside className="space-y-4">{session.model.adapter_id === 'msgnet' && <Info label="Internal graph semantics">{MSGNET_GRAPH_NOTICE}</Info>}<Info label="Source">Validated Session v2 · no browser inference</Info><Info label="Graph tensor">{context ? `${preferredGraph(context.graphs)} · ${context.node_count} nodes · ${edges.length} directed edges` : 'Unavailable'}</Info>{!specializedContext && <Info label="Generic graph context">Context type “{context?.type}” is preserved from Session v2 and rendered generically; no window/scale semantics are inferred.</Info>}{selected ? <Info label="Audited relation · locked">{variables[selected.source]} → {variables[selected.target]}</Info> : <Info label="Audited relation">No stored case evidence for this context</Info>}</aside>
    </div>
    <div role="alert" className="mx-auto max-w-[1400px] p-5">Results unavailable：This legacy audit import is available for graph traceability only. Summary requires independent performance.v1 results.</div>
  </div>;
}
