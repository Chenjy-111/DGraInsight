...
export function Hero() {
  ...
  return <section id="top" className="border-b border-line bg-white">
    <div className="mx-auto grid max-w-[1400px] gap-10 px-5 py-16 lg:grid-cols-[1.1fr_.9fr] lg:py-20">
      <div><div className="eyebrow mb-3">Audit forecast accuracy and response stability</div>
        ...
        <p className="mt-5 max-w-2xl text-[20px] leading-snug text-ink-800">Remove an edge. Assess forecast accuracy and response stability.</p>
        <p className="mt-4 max-w-2xl text-[14px] leading-relaxed text-ink-500">Compare forecasts before and after edge removal to see whether accuracy improves or degrades. Across audited samples, examine how consistently the response exceeds matched controls.</p>
      ...
      <div className="card p-6"><div className="eyebrow">Two questions for every edge</div><p className="mt-3 font-serif text-[25px] leading-snug text-[#263b59]">Does removing this edge improve prediction, and how stable is its effect?</p><div className="mt-6 grid grid-cols-3 gap-2 text-center text-[11px] font-semibold"><div className="rounded-lg bg-[#edf7f6] p-3 text-accent">Discover</div><div className="rounded-lg bg-[#eef2f7] p-3 text-[#263b59]">Test</div><div className="rounded-lg bg-[#f8f2e8] p-3 text-amber-800">Validate</div></div><p className="mt-4 text-[11px] leading-relaxed text-ink-400">Read forecast accuracy and response stability together. Formal support concerns the response relative to controls; accuracy direction comes from MAE and MSE.</p></div>
    </div>
  </section>;
}
