import { graphNodeLabels } from '@/data/graphSemantics';
import { ForecastPerformance } from './evidence/ForecastPerformance';
...
function QuickCasePresentation({ session, candidate, record, quick }: { session: AuditSessionV2; candidate: CandidateRelation; record: CaseEvidence; quick: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="card p-5">
      ...
      <div className="mt-4"><ForecastPerformance session={session} records={[record]}/></div>
      {intervention ? <PredictionReplayChart variables={(session.dataset as any).variables ?? []} target={session.model.adapter_id === 'msgnet' ? Math.max(0, ((session.dataset as any).variables as string[]).indexOf(String(session.dataset.target))) : candidate.target} baseline={sample.baseline_prediction} intervention={intervention} truth={sample.ground_truth}/> : <p className="mt-5 rounded-lg bg-amber-50 p-4 text-[12px] text-amber-900">Intervention prediction tensor is unavailable in this Session.</p>}
    ...
    <section className="card p-5">
      <div className="flex flex-wrap items-end justify-between gap-3"><div><div className="eyebrow">Matched controls</div><h3 className="mt-1 text-[17px] font-semibold text-[#263b59]">Response relative to matched controls</h3><p className="mt-2 text-[11px] text-ink-500">Every dot is one distinct eligible edge removal replay. This case compares response magnitude with controls; response stability across samples requires a multi-sample audit.</p></div><span className="font-mono text-[10px] text-ink-400">n = {record.controls.unique_count}</span></div>
...
function relationLabel(session: AuditSessionV2, source: number, target: number) { const variables = graphNodeLabels(session); return `${variables[source] ?? source} → ${variables[target] ?? target}`; }
