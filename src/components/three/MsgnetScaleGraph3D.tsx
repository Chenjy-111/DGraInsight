...
export function MsgnetScaleGraph3D(props: Props) {
  ...
  return <div data-testid="msgnet-3d-graph" className={props.immersive ? "relative h-[920px] w-full overflow-hidden bg-[#eef3f8]" : "relative h-[720px] w-full overflow-hidden rounded-lg bg-[#eef3f8]"}>
    <div className={`absolute ${props.immersive ? "left-5 right-5 lg:left-[330px] lg:right-[370px] top-32 lg:top-20" : "left-5 right-5 top-5"} z-10 flex flex-wrap items-start justify-between gap-2 rounded-xl border border-white/70 bg-white/75 p-4 shadow-sm backdrop-blur-md`}>
      <div><div className="text-[10px] font-semibold uppercase tracking-[.18em] text-[#718096]">Multi-scale graph laboratory</div><div className="mt-1 flex items-baseline gap-2"><span className="text-lg font-semibold text-[#233047]">Scale index {props.activeScale}</span><span className="text-[11px] text-[#7b879a]">period {props.contexts[props.activeScale].period} · {current.length} real relations</span></div></div>
    ...
    <div className={`pointer-events-none absolute bottom-20 ${props.immersive ? "left-5 lg:left-[330px]" : "left-5"} z-10 rounded-lg border border-white/80 bg-white/88 p-3 shadow-md backdrop-blur`}><div className="text-[9px] font-semibold uppercase tracking-wider text-[#758196]">Current graph evidence</div><div className="mt-2 text-[10px] text-[#6f7d90]">Strongest relation</div><div className="text-[11px] font-semibold text-[#167a77]">{strongest?`${props.variables[strongest.source]} → ${props.variables[strongest.target]} · ${strongest.weight.toFixed(3)}`:'—'}</div></div>
    <div className={`pointer-events-none absolute bottom-5 ${props.immersive ? "left-5 right-5 lg:left-[330px] lg:right-[370px]" : "right-5"} z-10 rounded-lg border border-white/80 bg-white/88 px-3 py-2 text-[10px] text-[#718096] shadow-md backdrop-blur`}>Scroll to page · Ctrl + scroll to zoom · drag to rotate</div>
    <Canvas camera={{position:[0,4.8,10.5],fov:40}} dpr={[1,1.75]} gl={{antialias:true}}>
      ...
      <CameraRig active={props.activeScale} count={props.graphs.length} spacing={4.8} overview={overview}/><ScaleLaboratory {...props}/><ScrollFriendlyOrbitControls />
    </Canvas>
  </div>;
}
...
function ScaleLayer({edges,index,active,x,positions,...props}:Props&{edges:GraphEdge[];index:number;active:boolean;x:number;positions:THREE.Vector3[]}){return <group position={[x,0,active?.35:-.4]} rotation={[-.07,0,-.026]} scale={active?1.04:.76}>
...
function Edge3D({edge,variables,active,selected,a,b,onClick}:{edge:GraphEdge;variables:string[];active:boolean;selected:boolean;a:THREE.Vector3;b:THREE.Vector3;onClick:()=>void}){const particle=useRef<THREE.Mesh>(null),[hover,setHover]=useState(false),bend=new THREE.Vector3((a.x+b.x)/2,(a.y+b.y)/2,.22+a.distanceTo(b)*.12);useFrame(({clock})=>{if(!particle.current||!active)return;particle.current.position.copy(new THREE.QuadraticBezierCurve3(a,bend,b).getPoint((clock.elapsedTime*(.18+edge.weight*.25)+edge.source*.13)%1))});return <group onClick={e=>{e.stopPropagation();onClick()}} onPointerOver={e=>{e.stopPropagation();setHover(true);document.body.style.cursor='pointer'}} onPointerOut={()=>{setHover(false);document.body.style.cursor='default'}}><QuadraticBezierLine start={a} end={b} mid={bend} color={selected?'#cf503d':'#16827f'} lineWidth={hover||selected?4:1.1+edge.weight*2.8} transparent opacity={active?.88:.14}/>{active&&<mesh ref={particle}><sphereGeometry args={[selected?.055:.035,10,10]}/><meshBasicMaterial color={selected?'#ef8a72':'#63c8c2'}/></mesh>}{hover&&<Billboard position={bend}><Html center distanceFactor={8} style={{pointerEvents:'none'}}><div className="min-w-[220px] rounded-xl border border-[#9bc9c6] bg-white/95 p-3 text-[10px] shadow-xl backdrop-blur-sm"><div className="font-semibold uppercase tracking-[.14em] text-[#176e69]">Directed relation</div><div className="mt-2 flex items-center gap-2 text-[13px] font-bold text-[#263b59]"><span><span className="mr-1 text-[9px] uppercase tracking-wide text-[#718096]">Source</span>{variables[edge.source]}</span><span className="text-lg text-[#16827f]">→</span><span><span className="mr-1 text-[9px] uppercase tracking-wide text-[#718096]">Target</span>{variables[edge.target]}</span></div><div className="mt-2 border-t border-[#dce8e7] pt-2"><div className="flex justify-between"><span>Weight</span><b className="font-mono">{edge.weight.toFixed(4)}</b></div><div className="mt-0.5 flex justify-between"><span>Rank</span><b>#{edge.rank}</b></div></div><div className="mt-2 text-[9px] leading-snug text-[#718096]">Reverse direction is a separate relation.</div></div></Html></Billboard>}</group>}
