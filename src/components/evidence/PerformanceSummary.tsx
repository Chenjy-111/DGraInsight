  ...
  return <div className="performance-summary space-y-5" data-testid="performance-summary">
    ...
    <section className="card p-5">
      ...
      {chosen ? <ReactECharts key={`${sampleId}:${context}:${source}:${target}:${scope}:${metric}:${axis}`} notMerge option={{
        ...
        series: [{ type: axis === 'samples' ? 'scatter' : 'line', smooth: false, connectNulls: false, showSymbol: true, symbolSize: axis === 'steps' ? 5 : 7, lineStyle: { color: '#64748b', width: 1.5 },
          data: points.map((p,i) => { const delta = p.after === undefined ? null : errorDelta(p.before, p.after), direction = p.after === undefined ? 'unavailable' : metricDirection(p.before, p.after, data.thresholdFloor); return { value: [p.x, delta], itemStyle: { color: direction === 'unchanged' ? '#64748b' : direction === 'degraded' ? '#c95445' : direction === 'improved' ? '#16827f' : '#94a3b8', borderColor: point === i ? '#e8a33f' : p.current ? '#263b59' : undefined, borderWidth: point === i || p.current ? 2 : 0 } }; }),
