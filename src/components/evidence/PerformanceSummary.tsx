    ...
    <section className="card p-5">
      ...
      {chosen ? <ReactECharts key={`${sampleId}:${context}:${source}:${target}:${scope}:${metric}:${axis}`} notMerge option={{
        ...
        series: [{ type: 'line', smooth: false, connectNulls: false, showSymbol: true, symbolSize: axis === 'steps' ? 5 : 7, lineStyle: { color: '#64748b', width: 1.5 },
          data: points.map((p,i) => { const delta = p.after === undefined ? null : errorDelta(p.before, p.after); return { value: [p.x, delta], itemStyle: { color: delta === null || delta === 0 ? '#64748b' : delta > 0 ? '#c95445' : '#16827f', borderColor: point === i ? '#e8a33f' : p.current ? '#263b59' : undefined, borderWidth: point === i || p.current ? 2 : 0 } }; }),
