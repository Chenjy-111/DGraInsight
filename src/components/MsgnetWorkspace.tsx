...
import { loadMsgnetCatalog, type MsgnetCatalog, type MsgnetSample } from '@/data/msgnetLoader';
...
export function MsgnetDataWorkspace() {
  ...
  useEffect(() => {
    ...
    if(index < 0 || !catalog.samples[index].contexts[selection.contextIndex]?.adaptive[selection.source]?.[selection.target]) {clearSelection('MSGNet');return;}
    setSampleId(index);setScale(selection.contextIndex);
  }, [selection,catalog]);
  ...
  const directionPair = selected ? [
    { source: selected.source, target: selected.target },
    { source: selected.target, target: selected.source },
  ] : [];
  ...
  const changeScale = (value: number) => {
    ...
    else if(selected) clearSelection('MSGNet');
  };
  ...
  return <div className="border-b border-line bg-white"><div data-testid="msgnet-workspace" className={immersive ? "msgnet-immersive relative min-h-[920px] w-full overflow-hidden" : "mx-auto grid max-w-[1400px] gap-6 px-5 py-14 lg:grid-cols-[300px_minmax(0,1fr)]"}>
    <aside className={immersive ? "relative mx-5 my-5 lg:absolute lg:left-5 lg:top-20 lg:m-0 z-30 max-h-[800px] lg:w-[300px] space-y-5 overflow-y-auto rounded-xl bg-white/95 p-5 shadow-xl" : "space-y-5"}><Group label="Case"><div className="space-y-2.5"><Field label="Sample"><Select value={sampleId} onChange={changeSample} options={catalog.samples.map((item,value)=>({value,label:`sample ${item.sample_index}`}))} ariaLabel="MSGNet graph sample"/></Field><Field label="Dataset"><Select value="ETTh1" onChange={() => {}} options={[{ value: 'ETTh1', label: 'ETTh1 · 7 vars' }]} ariaLabel="MSGNet dataset"/></Field><Field label="Target variable"><Select value={variable} onChange={setVariable} options={catalog.variables.map((label, value) => ({ value, label }))} ariaLabel="MSGNet target variable"/></Field><Field label="Prediction horizon"><Tabs value={96} onChange={() => {}} options={[{ value: 96, label: '96' }]} size="sm"/></Field></div></Group><Group label="View"><Tabs value={view} onChange={setView} options={[{ value: 'forecast', label: 'Forecast' }, { value: 'graph', label: 'Dynamic graph' }]} size="sm" wrap/></Group>{view === 'graph' && <Group label="Graph"><Slider label="Scale index" value={scale} min={0} max={2} onChange={changeScale} format={value => `#${value}`}/><Field label="Layout"><Tabs value={layout} onChange={setLayout} options={[{ value: 'heatmap', label: 'Matrix' }, { value: '3d', label: '3D scales' }]} size="sm" wrap/></Field></Group>}{view === 'graph' && <div className="mt-5 rounded-xl border-2 border-[#16827f] bg-[#edf7f6] p-4 shadow-[0_10px_28px_-18px_rgba(22,130,127,0.8)]"><div className="text-[15px] font-bold text-[#145f5c]">Select direction</div><p className="mt-1 text-[11px] leading-relaxed text-ink-600">Select an edge in the graph, then choose which direction to remove. The reverse direction is a separate experiment.</p>{selected ? <div className="mt-3 grid gap-2" role="group" aria-label="MSGNet relation direction">{directionPair.map(option => { const active = selected.source === option.source && selected.target === option.target; return <button key={`${option.source}-${option.target}`} type="button" aria-pressed={active} onClick={() => pick(option.source, option.target)} className={`rounded-lg border-2 px-4 py-3 text-left text-[15px] font-bold transition ${active ? 'border-[#16827f] bg-[#16827f] text-white shadow-md' : 'border-[#9bc9c6] bg-white text-[#263b59] hover:border-[#16827f] hover:bg-[#f7fcfb]'}`}><span className="text-[10px] font-semibold uppercase tracking-wide opacity-70">Source</span> {nodeLabels[option.source]} <span className="mx-1 text-lg">→</span> <span className="text-[10px] font-semibold uppercase tracking-wide opacity-70">Target</span> {nodeLabels[option.target]}</button>; })}</div> : <div className="mt-3 rounded-lg border border-dashed border-[#78b7b3] bg-white/70 px-3 py-4 text-center text-[12px] font-semibold text-[#176e69]">Select an edge in the graph first</div>}</div>}</aside>
    <main className={immersive ? "relative min-h-[920px] w-full" : "card min-h-[540px] p-5"}>{view === 'forecast' ? <div><div className="mb-3 flex items-center justify-between gap-3"><div className="flex items-center gap-3"><h3 className="whitespace-nowrap text-[15px] font-semibold">Forecast · {catalog.variables[variable]} <span className="text-ink-400">(ETTh1)</span></h3><Select value={sampleId} onChange={changeSample} options={Object.keys(catalog.samples).map(Number).map(value => ({ value, label: `sample ${value}` }))} ariaLabel="MSGNet sample"/></div><span className="data-num whitespace-nowrap text-[12px] text-ink-400">MSE {sample.metrics.mse.toFixed(6)} · MAE {sample.metrics.mae.toFixed(6)}</span></div><ForecastChart sample={adapted} variable={variable} windowIdx={scale} showPatchBoundary={false}/><p className="mt-3 text-[12.5px] leading-relaxed text-ink-400">The curve is a stored checkpoint output. Scale selection is independent of forecast residuals.</p></div> : <><div className={immersive ? "pointer-events-none absolute left-5 right-5 lg:left-[330px] lg:right-8 top-7 z-20 flex flex-wrap items-baseline justify-between gap-2" : "flex flex-wrap items-baseline justify-between gap-3"}><div><h3 className="text-[15px] font-semibold">Internal scale graph · scale_index {scale}</h3><p className="mt-1 text-[11px] font-semibold text-[#176e69]">Select a directed edge: source → target. The reverse direction is a different relation. Matrix rows = source; columns = target.</p></div><span className="data-num text-[12px] text-ink-400">current sample FFT period {context.period} · stored mixing weight {formatContribution(context.scale_contribution)}</span></div>{layout === 'heatmap' ? <Matrix matrix={context.adaptive} variables={nodeLabels} selected={selected} onPick={pick}/> : <MsgnetScaleGraph3D immersive={immersive} variables={nodeLabels} contexts={sample.contexts} graphs={sample.contexts.map(item => matrixEdges(item.adaptive))} activeScale={scale} selectedEdge={selected} onSelectScale={changeScale} onSelectEdge={(edge, selectedScale) => pick(edge.source, edge.target, selectedScale)}/>}{!immersive && <p className="mt-3 text-[12.5px] leading-relaxed text-ink-400">{MSGNET_GRAPH_NOTICE}</p>}</>}</main>
  </div></div>;
}
