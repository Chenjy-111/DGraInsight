...
export function CitationSection() {
  return <Section id="cite" eyebrow="Citation & code" title="Models & adapters" intro="Original references for built-in adapters, plus a path for your own model.">
    ...
    <div className="mt-5 rounded-xl border border-line bg-white p-5 text-[11px] leading-relaxed text-ink-500"><b className="text-ink-800">Attribution.</b> Model contributions belong to the original authors. DGraInsight is an independent relation-removal analysis system.</div>
  </Section>;
}

function CustomModelCard() {
  return <article className="card flex h-full flex-col overflow-hidden border-dashed border-accent/40"><header className="flex flex-1 flex-col border-b border-line bg-[#edf7f6] p-5"><div className="eyebrow">Your Model · custom adapter</div><h3 className="mt-2 text-[19px] font-semibold leading-snug md:min-h-[104px]">Bring your own forecasting model</h3><p className="mt-3 text-[12px] leading-relaxed text-ink-500 md:min-h-[84px]">Import existing results, connect your prediction and removal functions, or provide a reusable model backend.</p><p className="mt-2 text-[11px] font-semibold text-ink-700 md:min-h-[32px]">Model-defined architecture</p><p className="mt-1 text-[10px] text-ink-400 md:min-h-[28px]">DGraInsight context: model-defined</p><div className="mt-auto pt-4"><a className="inline-block rounded-lg bg-[#16827f] px-3 py-2 text-[10px] font-semibold text-white" href="https://github.com/Chenjy-111/DGraInsight/tree/main/offline_app">Evaluation guide</a></div></header><div className="h-[276px] shrink-0 p-5"><div className="eyebrow mb-2">Shared workflow</div><p className="rounded-md bg-paper p-3 font-mono text-[11px] leading-relaxed text-accent">Results or functions → Evaluation → Exploration</p></div></article>;
}
...
function PaperCard({model,context,title,authors,venue,paper,code,bibtex}:{model:string;context:string;title:string;authors:string;venue:string;paper?:string;code:string;bibtex:string}) {
  return <article className="card flex h-full flex-col overflow-hidden"><header className="flex flex-1 flex-col border-b border-line bg-[#fafbfd] p-5"><div className="eyebrow">{model} · supported adapter</div><h3 className="mt-2 text-[19px] font-semibold leading-snug md:min-h-[104px]">{title}</h3><p className="mt-3 text-[12px] leading-relaxed text-ink-500 md:min-h-[84px]">{authors}</p><p className="mt-2 text-[11px] font-semibold text-ink-700 md:min-h-[32px]">{venue}</p><p className="mt-1 text-[10px] text-ink-400 md:min-h-[28px]">DGraInsight context: {context}</p><div className="mt-auto flex flex-wrap gap-2 pt-4">{paper&&<a className="rounded-lg bg-[#263b59] px-3 py-2 text-[10px] font-semibold text-white" href={paper} target="_blank" rel="noreferrer">Official paper</a>}<a className="rounded-lg border border-[#263b59] bg-white px-3 py-2 text-[10px] font-semibold text-[#263b59]" href={code} target="_blank" rel="noreferrer">Official code</a></div></header><div className="h-[276px] shrink-0 p-5"><div className="eyebrow mb-2">BibTeX</div><pre className="max-h-[220px] overflow-auto rounded-md bg-paper p-3 font-mono text-[10px] leading-relaxed text-ink-700">{bibtex}</pre></div></article>;
}
