...
- `THIRD_PARTY_NOTICES.md`: provenance and redistribution boundary for the
  reviewer-supplied Microsoft source.
...
Clone Microsoft StemGNN at the pinned revision while preserving LF line endings:

```text
git clone --no-checkout https://github.com/microsoft/StemGNN.git official_stemgnn
git -C official_stemgnn config core.autocrlf false
git -C official_stemgnn checkout dc7dea6842c20c5dcece2f18b435e26770b3421c
```

Set `source_root` to that checkout. The adapter verifies the upstream
`models/base_model.py` hash, applies the two required PyTorch FFT API
substitutions in memory, and verifies the generated bytes against SHA-256
`ae87de6c4ed4c011ec35bb28bcea194b291f9b14ed3ae306ef19faac38eb9f25`
before executing them. It never writes or packages a modified copy of the
Microsoft source. The submitted checkpoint and dataset are under the release's
`paper_resources/` directory.
