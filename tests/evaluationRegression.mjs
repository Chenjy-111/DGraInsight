...
const { validateEvaluation, crossSampleConsistency, metricChange, displayedMetrics, rankedRemovals } = await import('data:text/javascript;base64,' + Buffer.from(js).toString('base64'));
...
assert.equal(consistency.find(group => group.selected).available, 2);
assert.equal(consistency.find(group => group.selected).assessable, true);
for (const group of consistency) for (const metric of ['mae','mse']) assert.equal(Object.values(group.directions[metric]).reduce((a,b)=>a+b,0), group.available);
const oneSample = structuredClone(d); oneSample.records = oneSample.records.filter(record => record.sampleId === '0');
assert.equal(crossSampleConsistency(oneSample, oneSample.records[0]).find(group => group.selected).assessable, false);
