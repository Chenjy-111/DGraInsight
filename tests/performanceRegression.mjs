...
const {changePercent,conclusion,errorDelta,mean,metricDirection}=await import('data:text/javascript;base64,'+Buffer.from(js).toString('base64'));
...
assert.ok(Number.isNaN(changePercent(0,1)));
assert.ok(Math.abs(errorDelta(1,.9)+.1)<1e-12);
assert.ok(Math.abs(errorDelta(1,1.1)-.1)<1e-12);
assert.ok(Math.abs(changePercent(1,.9)+10)<1e-12);
assert.ok(Math.abs(changePercent(1,1.1)-10)<1e-12);
...
assert.equal(metricDirection(1,1.002),'degraded');
assert.equal(metricDirection(1,1.001),'unchanged');
assert.equal(metricDirection(1,.999),'unchanged');
assert.equal(metricDirection(NaN,1),'unavailable');
