...
try {
  ...
  await work.waitFor();
  assert.equal(await work.locator('[data-change]').count(), 4);
  await work.locator('[data-change] button').first().click();
  await page.getByLabel('Evaluation sample', {exact:true}).selectOption('1');
  assert.equal(await work.locator('[data-change] button[aria-pressed=true]').count(),1);
  await page.getByLabel('Evaluation output',{exact:true}).selectOption('1');
  ...
  await page.getByLabel('Evaluation sample',{exact:true}).selectOption('1');
  assert.equal(await work.locator('[data-change] button[aria-pressed=true]').count(),0);
